...
{
    int N = 100;
    DEBUG(DB_GPU, std::cout << "Creating simulation" << std::endl);
    Simulation sim = Simulation(N);

    DEBUG(DB_GPU, std::cout << "Initializing random densities" << std::endl);
    sim.initialize_densities_random();
   
    DEBUG(DB_GPU, std::cout << "Writing densities" << std::endl);
    sim.write_densities_to_file("densities.csv");

    DEBUG(DB_GPU, std::cout << "Running 1 primary ray" << std::endl);
    sim.run_serial(1);

    DEBUG(DB_GPU, std::cout << "Writing doses" << std::endl);
    sim.write_doses_to_file("doses.csv");
